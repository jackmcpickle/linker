  ...
  return (
    <tr id={`link-${link.token}`} class="border-b border-zinc-100 align-top">
      <td class="px-4 py-3">
        ...
        {link.notes ? (
          <div class="text-xs text-zinc-500 mt-0.5">{link.notes}</div>
        ) : null}
      ...
      <td class="px-4 py-3">
        ...
        {s === "active" ? (
          <div class="text-xs text-zinc-500 mt-1">
            expires <time datetime={isoAt(link.expiresAt)} data-time-rel>{relative(link.expiresAt)}</time>
          </div>
        ) : null}
      </td>
      <td class="px-4 py-3 text-xs text-zinc-500">
        ...
        {link.lastAccessedAt ? (
          <div class="text-zinc-400">
            <time datetime={isoAt(link.lastAccessedAt)} data-time-rel>{relative(link.lastAccessedAt)}</time>
          </div>
        ) : (
